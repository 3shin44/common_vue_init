<template>
  <div class="common-table">
    ...
    <div class="base-table-body">
      <el-table v-loading="loading" :data="displayData" v-bind="elTableAttrs" @row-click="rowClick" empty-text="暫無資料"
        @sort-change="sortChange">
        <template v-for="(item, index) in getHeaders">
          <!-- slot -->
          <el-table-column v-if="item.type === 'button'" :label="item.label" :width="item.width"
            :min-width="item.minWidth" :key="index" v-bind="item.colAttrs">
            <template slot-scope="scope">
              <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">編輯</el-button>
            </template>
          </el-table-column>
          <!-- default column -->
          <el-table-column v-else :prop="item.data" :label="item.label" :width="item.width" :min-width="item.minWidth"
            :key="index" v-bind="item.colAttrs">
          </el-table-column>
        </template>
      </el-table>
    </div>
    ...
    <div class="base-table-footer pt-2">
      ...
      <div v-if="showPagination">
        <el-pagination class="d-flex justify-content-center" layout="prev, pager, next" :total="tableData.length"
          :current-page.sync="currentPageNo" :page-size="numberPerPage">
        </el-pagination>
      </div>
    </div>
  </div>
